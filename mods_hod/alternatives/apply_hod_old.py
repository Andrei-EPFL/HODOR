...
class ModelClass():
	...
	def __init__(self, config_file, halo_files):
		...
		if len(self.parameters_names) != self.npar:
			print("ERROR: Check the npar and model_params_names in the config file")
			sys.exit(1)
		...
		print('\nINFO: The number of seeds to populate galaxies is: ', self.num_seeds)
		...
		for j, halo_cat in enumerate(self.halo_cat_list):
			for i in range(num_seeds):
				if (i == 0) and (j == 0):
					...
					meas_num_dens = len(return_dict[os.path.basename(self.halo_files[j]) + str(seed0 + i*100 + j)]) / (self.box_size ** 3)
